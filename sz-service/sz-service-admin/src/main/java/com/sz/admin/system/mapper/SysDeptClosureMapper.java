...
public interface SysDeptClosureMapper extends BaseMapper<SysDeptClosure> {
    ...
    @Delete(" DELETE " +
            ...
            " sys_dept_closure d " +
            " WHERE" +
            " d.ancestor_id = #{nodeId} and t.descendant_id = d.descendant_id " +
            " ) " +
            " and " +
     ...
     * @return
     */
    @Select(" SELECT ancestor_id,descendant_id,depth FROM sys_dept_closure " +
            " WHERE EXISTS (" +
            "    SELECT 1 " +
            "    FROM sys_dept_closure d " +
            "    WHERE d.ancestor_id = #{nodeId} " +
            "    AND sys_dept_closure.descendant_id = d.descendant_id" +
            ")" +
            " AND EXISTS (" +
            "    SELECT 1 " +
            "    FROM sys_dept_closure a " +
            "    WHERE a.descendant_id = #{nodeId} " +
            "    AND a.ancestor_id != a.descendant_id " +
     ...
     * @return
     */
    @Insert("INSERT INTO sys_dept_closure ( ancestor_id, descendant_id, depth ) SELECT" +
            " super.ancestor_id," +
            " sub.descendant_id,(" +
            " super.depth + sub.depth + 1 " +
            " ) " +
            " FROM" +
            " sys_dept_closure super" +
            " CROSS JOIN sys_dept_closure sub " +
            " WHERE" +
            " super.descendant_id = #{newNodeId} " +
            " AND sub.ancestor_id = #{nodeId}")
