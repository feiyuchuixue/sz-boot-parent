 ...
 * MySQL 数据权限方言实现。
 * <p>
 * 使用 MySQL 专属语法：反引号包标识符、JSON_CONTAINS 函数等。
 * SQL 拼接字符串与 Phase A 的 SimplePermissionDialect 完全等价。
 *
 * @since Phase B
 */
@Configuration
public class MysqlPermissionDialect extends AbstractPermissionDialect {
    ...
    protected String buildUserUnitDeptSql(String table, String field, Collection<Long> deptList) {
        ...
        return new StringBuilder()
                .append(" EXISTS ( SELECT 1 FROM `sys_user_dept` ")
                .append("JOIN `sys_dept` ON `sys_user_dept`.`dept_id` = `sys_dept`.`id` ")
                .append("WHERE `sys_user_dept`.`dept_id` IN ").append(sqlParams)
                .append(" AND `sys_dept`.`del_flag` = 'F' ")
                .append("AND `").append(table).append("`.`").append(field).append("` = `sys_user_dept`.`user_id`) ")
                .toString();
    }
    ...
    protected String buildAllowAdminViewSql(String table) {
        return new StringBuilder()
                .append(" EXISTS (SELECT 1 FROM `sys_user` WHERE `sys_user`.`id` = `")
                .append(table).append("`.`").append(FIELD_CREATE_ID)
